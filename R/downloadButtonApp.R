...
#' @export
downloadButtonApp <- function(id) {
  ui <- bslib::page_sidebar(
    title = "Test Download",
    sidebar = bslib::sidebar("side_panel", width = 400,
      downloadButtonInput("download"), # plot_table, inputs for Plots or Tables
      downloadButtonOutput("download") # downloadButton, filename
    ),
    bslib::card(
      downloadButtonUI("download")
    ),
    bslib::card(
      bslib::card_header("Download Preview"),
      downloadButtonShow("download")    # Only for Preview of downloadButtonApp().
    )
  )
  server <- function(input, output, session) { 
    ...
    downloadButtonServer("download", download_list)
  }
  shiny::shinyApp(ui, server)
}
#' @rdname downloadButtonApp
#' @export
downloadButtonServer <- function(id, download_list) {
...
#' @rdname downloadButtonApp
#' @export
downloadButtonInput <- function(id) {
  ns <- shiny::NS(id)
...
#' @rdname downloadButtonApp
#' @export
downloadButtonOutput <- function(id) {
  ns <- shiny::NS(id)
  list(
    shiny::h5("Download:"),
    bslib::layout_columns(
...
downloadButtonShow <- function(id) {
  ns <- shiny::NS(id)
